 ...
 */
class NPR_WP_Widget_Shopping_Cart extends WP_Widget {

	/**
	 * Widget Constuctor
	 */
	function NPR_WP_Widget_Shopping_Cart() {

		$widget_ops = array(
			'classname'   => 'widget_npr_wpsc_shopping_cart',
			'description' => __( 'Shopping Cart Widget', 'wpsc' )
		);

		$this->WP_Widget( 'npr_wpsc_shopping_cart', __( 'Shopping Cart', 'wpsc' ), $widget_ops );
	...
	function widget( $args, $instance ) {
		...
		if ( ( $cache_enabled == true ) && ( !defined( 'DONOTCACHEPAGE' ) || ( constant( 'DONOTCACHEPAGE' ) !== true ) ) ) {
			echo '<!-- npr_shopping_cart_widget --><div id="sliding_cart" class="shopping-cart-wrapper npr-cart">';
			...
			echo '<!-- npr_shopping_cart_widget --><div id="sliding_cart" class="shopping-cart-wrapper" ' . $display_state . '>';
			$path = get_bloginfo('template_directory')."/php/nprwidgets/".'npr_wpsc-cart_widget.php';
			echo "<!-- path to include = $path -->";
...
add_action( 'widgets_init', create_function( '', 'return register_widget("NPR_WP_Widget_Shopping_Cart");' ) );
